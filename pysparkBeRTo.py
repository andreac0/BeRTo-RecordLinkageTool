...
def data_preparation(dataset, identifier, name_entity, address, city, postal_code, country):
  ...
  '''
  # if address != 'NA':
  #     exploded_dataset = dataset.withColumn('name', sf.explode(sf.split(col("name"), "-")))
  #     dataset = dataset.unionByName(exploded_dataset).dropDuplicates()
...
def stringMatching(dataset1, identifier1, dataset2, identifier2, similarity_on_address = True, \
                  ...
                  city = 'NA', postcode = 'NA', use_address = True):
  ...
  dataset1 = dataset1.repartition("country")
  ...
  dataset2 = dataset2.repartition("country2")
  ...
  if similarity_on_address == True:
    ...
    if city != 'NA' and postcode != 'NA':
      ...
      cond1 = (
        (split1.name == dataset2.name2) & \
        # (sf.levenshtein(split1.name, dataset2.name2) <= 3) &\
        (split1.country == dataset2.country2) &\
        ((sf.levenshtein(split1.street, dataset2.street2) <= diff_group1) | (split1.city == dataset2.city2) | (split1.postcode == dataset2.postcode2)))
      
      cond2 = (
        (split2.name == dataset2.name2) & \
        # (sf.levenshtein(split2.name, dataset2.name2) <= 3) &\
        (split2.country == dataset2.country2) &\
        ((sf.levenshtein(split2.street, dataset2.street2) <= diff_group2) | (split2.city == dataset2.city2) | (split2.postcode == dataset2.postcode2)))
      
      cond3 = (
        (split3.name == dataset2.name2) & \
        # (sf.levenshtein(split3.name, dataset2.name2) <= 3) &\
        (split3.country == dataset2.country2) &\
        ((sf.levenshtein(split3.street, dataset2.street2) <= diff_group3) | (split3.city == dataset2.city2) | (split3.postcode == dataset2.postcode2)))
  ...
  if similarity_on_address == False:
    ...
    if city != 'NA' and postcode != 'NA':
      ...
      cond = (
        (dataset1.name == dataset2.name2) & \
        (dataset1.country == dataset2.country2) &\
      ...
      cond = (
        # (sf.levenshtein(dataset1.name, dataset2.name2) <= 3) &\
        (dataset1.name == dataset2.name2) & \
        (dataset1.country == dataset2.country2) &\
        (dataset1.street == dataset2.street2) &\
...
def fuzzyNameMatching(spark, dataset1_original, dataset2_original, config):
  
  ## Set configurations
  if config["similarity_level"] == 1:
      first_split = 12   # Length of the address to create the first group
      diff_group1 = 4    # Dissimilarities accepted for the first group
    
      second_split = 25  # Length of the address to create the second and third groups
      diff_group2 = 6    # Dissimilarities accepted for the second group 
      diff_group3 = 8    # Dissimilarities accepted for the third group, with length(address) > second split
  elif config["similarity_level"] == 2:
      ...
      first_split = 8   # Length of the address to create the first group
      diff_group1 = 4    # Dissimilarities accepted for the first group
    
      second_split = 13  # Length of the address to create the second and third groups
      diff_group2 = 7    # Dissimilarities accepted for the second group 
      diff_group3 = 10    # Dissimilarities accepted for the third group, with length(address) > second split
    
  ## Load Dictionaries

  # Official abbreviations

  abbreviations = pd.read_csv('Data/dictionaries/dictionary_abbreviations.csv')
  ...
  fuzzy_dict = pd.read_csv('Data/dictionaries/fuzzywuzzy_dictionary.csv')
  ...
  if config["type_country"] != config["type_country2"]:

    isocodes = pd.read_csv('Data/dictionaries/isocodes.csv')[['name', 'alpha-2', 'alpha-3']]
  ...
  dataset1_name_match = data_preparation(dataset1_original,  identifier = config["identifier_1"], \
                                     name_entity = config["name_1"], address = config["street_1"], \
                                     city = config["city_1"], postal_code = config["post_1"], country = config["country_1"])

  # Convert country into isocode if needed:

  if config["type_country"] != config["type_country2"]:

    dataset1_name_match = convert_isocode(dataset1_name_match, isocodes, config["type_country"])
  ...
  if config["use_fuzzy_dictionary"]:
  ...
  #### Apply attribute cleaning function
  dataset2_name_match = data_preparation(dataset2_original,  identifier = config["identifier_2"], \
                                     name_entity = config["name_2"], address = config["street_2"], \
                                     city = config["city_2"], postal_code = config["post_2"], country = config["country_2"])

  # Convert country into isocode if needed:

  if config["type_country"] != config["type_country2"]:
    dataset2_name_match = convert_isocode(dataset2_name_match, isocodes, config["type_country2"])
  ...
  #-------------------------------
  if config["street_1"] == 'NA':
    address_similarity = False

  c1 = dataset1.select('country').drop_duplicates().rdd.map(lambda x: x['country']).collect()
  c2 = dataset2.select('country').drop_duplicates().rdd.map(lambda x: x['country']).collect()
  c3 = list(dict.fromkeys(c1+c2))
  
  data1 = dataset1.filter(col('country') == c3[0])
  data2 = dataset2.filter(col('country') == c3[0])
  
  name_matches = stringMatching(data1, config["identifier_1"], data2, config["identifier_2"], \
                                config["address_similarity"], first_split, second_split, diff_group1, \
                                diff_group2, diff_group3, config["city_1"], config["post_1"])
  for i in c3[1:]:
        data1 = dataset1.filter(col('country') == i)
        data2 = dataset2.filter(col('country') == i)
  
        name_matches = name_matches.unionByName(stringMatching(data1, config["identifier_1"], data2, config["identifier_2"], \
                                config["address_similarity"], first_split, second_split, diff_group1, \
                                diff_group2, diff_group3, config["city_1"], config["post_1"]))
  ...
  if config["add_column_score"] == True:
      name_matches = compute_score(name_matches, dataset1_name_match, \
                                   dataset2_name_match, config["identifier_1"], config["identifier_2"],\
                                   config["street_1"], config["city_1"], config["post_1"]).dropDuplicates()
