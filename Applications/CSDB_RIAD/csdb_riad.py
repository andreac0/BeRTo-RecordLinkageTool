...
spark = (
    SparkSession.builder.appName("csdb_name")
...
country_attribute = 'isocode2'
 ...
 #-------------
# Load data and select all the relevant attributes
csdb = spark.read.parquet("/xdata/corporate/csdb/csdb_inst_fact_ncbex_revsd_optimize/")\
                 .select(col("issuerexternalcode").alias('issuer_id'),
                         'issuername',
                         col('idircountry').alias('country'))\
                 .filter(col("va_securitystatus").startswith("1") # the instrument is alive
                        )\
                 .filter(col("isclashgroup") == "M" # main group of instruments
                        )\
                 .dropDuplicates()

        
        
dataset2_original = csdb.join(dataset1_original, [csdb.issuer_id == dataset1_original.entty_riad_cd],'left_anti')\
                         .select(col('issuername').alias('id'), 'issuername', 'country')\
                         .dropDuplicates()

 # csdb attribute names

id_data2 = 'id'
name_data2 = 'issuername'
country_2 = 'country'
street_data2 = 'NA'
city_data2 = 'NA'
pstl_data2 = 'NA'
...
disc_lab = '' # only data lab allowed, no corporate store
...
name_matches.write.mode('overwrite').parquet('csdb_riad_name')
